...
c.orders={"popup":a.isBrowser("chrome")?["Ftn","H5CPopupMail","PPFtn","FlashH5PopupMail","FlashPopupFMail","RawinputPopupMail"]:["Ftn","H5CPopupMail","FlashH5PopupMail","FlashPopupFMail","ActivexPopupMail","RawinputPopupMail"],"drag":["H5CDragFMailCROS","FlashH5DragMailCROS","Html5DragCROS"],"paste":["Ftn"]};
...
this.chipUpdateFileSize_(i);
h.onprocess.apply(this,arguments);
...
this.chipUpdateFileSize_(e);
d.onprocess.apply(this,arguments);
...
var h=getTop(),g=this,f={},i,j=!h.isHttp();
for(key in e)
{
...
else if(key==="request_url"&&j)
{
e[key]=e[key].replace("http://","https://");
}
if(h.isArr(i))
{
for(iKey in i)
...
var f=this;
f._moActivex.postMessage({"cmd":"cmdUploadFile","file_res":+e.get("sFileRes"),"svr":e.get("sIP"),"port":c.isHttp()?80:443,"svr_key":e.get("sKey"),"sha":e.get("sSHA")});
e.set('nLocalID',e.get("sFileRes"));
...
else if(g.status==0&&/timeout/i.test(h.type))
{
return;
}
else{
f.onerror(e.set('sError',f.err('http',g.status)));
}
}
};
